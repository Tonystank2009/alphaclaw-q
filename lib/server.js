...
const { verifyToken } = require("./server/supabase");
const { createRateLimitMiddleware } = require("./server/rate-limit");
const { registerPaymentRoutes } = require("./server/routes/payments");
const { createGatewayCaller } = require("./server/gateway-call");

const gatewayCaller = createGatewayCaller({
...
app.use(async (req, res, next) => {
  ...
  if (token) {
    try { req.user = await verifyToken(token); } catch {}
  }
  next();
});
...
// Inject env vars into Q-branded HTML pages BEFORE static middleware
// (otherwise static serves the raw file with %%placeholders%% intact).
const _publicDir = path.join(__dirname, "public");
const _injectEnv = (filename) => (req, res) => {
  try {
    let html = fs.readFileSync(path.join(_publicDir, filename), "utf8");
    html = html
...
app.get("/login.html", _injectEnv("login.html"));
app.get("/subscribe.html", _injectEnv("subscribe.html"));
app.get("/dashboard.html", _injectEnv("dashboard.html"));
app.get("/chat.html", _injectEnv("chat.html"));
