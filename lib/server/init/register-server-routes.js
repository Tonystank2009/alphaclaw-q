...
const { registerOnrampRoutes } = require("../routes/onramp");
const { registerProfileRoutes } = require("../routes/profile");
const { registerEmailInboundRoutes } = require("../routes/email-inbound");
const { registerChatRoutes } = require("../routes/chat");
const { registerSmsRoutes } = require("../routes/sms");
...
const registerServerRoutes = ({
  ...
  registerEmailInboundRoutes({ app, gatewayCaller });
  registerChatRoutes({ app, gatewayCaller });
  registerSmsRoutes({ app, gatewayCaller });
  registerCallRoutes({ app });
  const { requireSubscription } = registerPaymentRoutes({
    app,
    agentsService,
    restartGateway,
  });

  // After auth: check subscription, then enforce per-resource rate limits.
  app.use("/setup", requireSubscription);
  app.use("/api", requireSubscription);

  // Token-based limits on LLM-generating endpoints; SMS/voice limits on those.
  // Each middleware checks budget for the resource the route consumes.
  app.use("/api/chat/send", createRateLimitMiddleware({ resource: "tokens" }));
  app.use("/api/sms/send", createRateLimitMiddleware({ resource: "sms" }));
  app.use("/api/call/place", createRateLimitMiddleware({ resource: "voice_seconds" }));

  // Catch-all (messages-resource) middleware for proxy routes that fan out to gateway.
  const rateLimitMiddleware = createRateLimitMiddleware({ resource: "messages" });
