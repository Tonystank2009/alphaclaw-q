...
const registerServerRoutes = ({
  ...
  registerEmailInboundRoutes({ app });
  registerChatRoutes({ app });
  registerSmsRoutes({ app });
  registerCallRoutes({ app });
  const { requireSubscription } = registerPaymentRoutes({ app, agentsService });
  const rateLimitMiddleware = createRateLimitMiddleware();

  // After auth: check subscription, then enforce rate limits on action routes.
  app.use("/setup", requireSubscription);
  app.use("/api", requireSubscription);
  // Rate-limit only the "doing" endpoints, not status/profile/payment lookups.
  app.use("/api/chat/send", rateLimitMiddleware);
  app.use("/api/sms/send", rateLimitMiddleware);
  app.use("/api/call/place", rateLimitMiddleware);
