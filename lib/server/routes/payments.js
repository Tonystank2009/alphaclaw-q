...
const registerPaymentRoutes = ({ app, agentsService, restartGateway }) => {
  ...
  const FOUNDER_PRICE_PER_DAY = process.env.FOUNDER_PRICE_PER_DAY || "1";
  const FOUNDER_PRICE_PER_MO = process.env.FOUNDER_PRICE_PER_MO || "30";
  const FUTURE_PRICE_PER_MO = process.env.FUTURE_PRICE_PER_MO || "60";
  ...
  app.post("/api/payments/test-claim", async (req, res) => {
    ...
    try {
      ...
      provisionUser({ userId: user.id, agentsService, restartGateway }).catch((err) =>
  ...
  app.post("/api/payments/webhook", async (req, res) => {
    ...
    if (newStatus) {
      ...
      if (newStatus === "active" || newStatus === "trialing") {
        provisionUser({ userId, agentsService, restartGateway }).catch((err) =>
          console.error("[provision] failed:", err.message),
        );
      }
    }
