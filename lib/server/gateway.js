...
const isOnboarded = () => {
  ...
  // If openclaw.json exists, the gateway was previously onboarded — the
  // marker file just got lost (volume swap, repo-restored state, etc).
  // Treat the presence of a valid-looking config as proof of onboarding.
  const configPath = path.join(OPENCLAW_DIR, "openclaw.json");
  if (fs.existsSync(configPath)) {
    writeOnboardingMarker("config_present_backfill");
    return true;
  }
  return false;
};
