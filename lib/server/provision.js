...
const kPersonalityConfig = {
  ...
  unhinged: {
    voiceId: process.env.VAPI_VOICE_UNHINGED || "Cole",
...
const provisionUser = async ({ userId, agentsService, restartGateway }) => {
  ...
  // 3b. Reload the gateway so the new agent is live for chat/SMS/email.
  if (typeof restartGateway === "function") {
    try {
      await restartGateway();
      console.log(`[provision] gateway reloaded for new agent ${agentId}`);
    } catch (err) {
      console.error("[provision] gateway reload failed:", err.message);
    }
  }
