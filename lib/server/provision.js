...
// New per-user agents need their own auth-profiles.json so OpenClaw can find
// the Anthropic key when running their session. The simplest approach: copy
// from the main agent's profile, which was set up during admin onboarding.
const copyAuthProfilesFromMain = (userAgentId) => {
  try {
    ...
    const mainProfile = pathMod.join(openclawDir, "agents", "main", "agent", "auth-profiles.json");
    const userProfile = pathMod.join(openclawDir, "agents", userAgentId, "agent", "auth-profiles.json");
    if (!fsMod.existsSync(mainProfile)) {
      console.warn(`[provision] main agent auth-profiles missing at ${mainProfile}`);
      return false;
    }
    fsMod.mkdirSync(pathMod.dirname(userProfile), { recursive: true });
    fsMod.copyFileSync(mainProfile, userProfile);
    console.log(`[provision] auth profiles copied to ${userAgentId}`);
    return true;
  } catch (err) {
    console.error(`[provision] auth profile copy failed: ${err.message}`);
    return false;
  }
};
...
const createAlphaclawAgent = (agentsService, userId, aiName) => {
  ...
  copyAuthProfilesFromMain(agentId);
  return agentId;
};
