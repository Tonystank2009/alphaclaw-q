...
const reloadEnv = () => {
  ...
  // NOTE: We intentionally do not delete process.env keys here. Railway (and
  // similar PaaS providers) inject env vars directly into the process, and the
  // local .env file is only used by alphaclaw to remember user-onboarded
  // values. Deleting platform-provided vars when they're absent from the local
  // .env produces "Env removed" cascades that blow up the running service.
